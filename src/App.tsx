...
const KARIM_CHAT_API = '/api/karim';
...
export default function App() {
  ...
  const [karimArtworkUrl, setKarimArtworkUrl] = useState<string | undefined>(isRasterArtwork(initialData.karimArtworkUrl) ? initialData.karimArtworkUrl : undefined);
  ...
  const deliverKarimMessages = async (msgs: string[]) => {
    for (let i = 0; i < msgs.length; i++) {
      ...
      setIsTyping(false);
      setMessages((prev) => [...prev, { id: `msg-karim-${Date.now()}-${i}`, sender: 'karim', text, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), status: 'read' }]);
  ...
  const handleSendMessage = async (text: string) => {
    ...
    const playerMsg: ChatMessage = { id: `msg-player-${Date.now()}`, sender: 'player', text, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), status: 'sent' };
    ...
    try {
      ...
      if (Array.isArray(data.newMemories) && data.newMemories.length > 0) {
        const formattedNew: MemoryItem[] = data.newMemories.map((m: { category: string; content: string; importance?: number }, i: number) => ({ id: `mem-${Date.now()}-${i}`, category: (m.category as MemoryItem['category']) || 'fact', content: m.content, createdAt: new Date().toISOString(), importance: m.importance ?? 3 }));
        setMemories((prev) => { const existingContents = new Set(prev.map((p) => p.content)); return [...prev, ...formattedNew.filter((n) => !existingContents.has(n.content))]; });
      }
      ...
      if (data.updatedRelationship) {
        ...
        setRelationship((prev) => {
          ...
          return { ...prev, closeness: newCloseness, affection: newAffection, trust: newTrust, stage: calculatedStage, stageName: stageInfo.name, statusText: typeof statusText === 'string' && statusText.trim() ? statusText : prev.statusText };
        });
      }
    } catch (err) {
      console.error('Failed to chat with Karim:', err);
      setIsTyping(false);
      setMessages((prev) => [...prev, { id: `msg-err-${Date.now()}`, sender: 'karim', text: 'Karim lagi susah nyambung. Coba lagi sebentar ya.', timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }]);
    }
  };
  ...
  const handleSelectEvent = useCallback(async (event: SchoolEvent) => {
    ...
    try {
      const res = await fetch('/api/initiate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ playerProfile, currentEvent: event, relationship }) });
      if (!res.ok) throw new Error(`Initiate API returned ${res.status}`);
      const data = await res.json();
      const initMsgs: string[] = Array.isArray(data.messages) ? data.messages.filter((message: unknown): message is string => typeof message === 'string' && message.trim().length > 0) : [event.starterPrompt];
      await deliverKarimMessages(initMsgs);
    } catch (err) { console.error('Failed to fetch initiate message:', err); setIsTyping(false); }
  }, [playerProfile, relationship]);

  const handleResetGame = () => {
    const defaultData = resetGameState();
    setMessages(defaultData.messages); setMemories(defaultData.memories); setRelationship(defaultData.relationship); setCurrentEvent(defaultData.currentEvent); setPlayerProfile(defaultData.playerProfile); setKarimArtworkUrl(undefined); setAudioEnabled(true); setAutoInitiateEnabled(true);
  };

  if (showSplash) {
    return <div className={`karim-splash ${splashLeaving ? 'is-leaving' : ''} fixed inset-0 z-50 flex items-center justify-center bg-[#f6e7c7]`}><div className="relative flex h-[78vh] max-h-[760px] w-[min(390px,88vw)] flex-col items-center justify-center overflow-hidden rounded-[34px] border-[7px] border-[#4c4145] bg-[#fff5df] shadow-[0_16px_0_rgba(76,65,69,.18)]"><div className="absolute inset-3 rounded-[27px] border-2 border-[#ead7b6]" /><div className="relative z-10 flex flex-col items-center gap-5 px-8 text-center"><div className="rounded-full border-4 border-[#dfc79f] bg-[#fffaf0] p-2 shadow-[0_5px_0_rgba(101,76,51,.12)]"><img src={KARIM_ARTWORK} alt="Karim" className="h-32 w-32 rounded-full object-cover sm:h-36 sm:w-36" /></div><div><p className="mb-1 text-[12px] font-extrabold uppercase tracking-[.3em] text-[#a1846c]">a message from</p><h1 className="text-5xl font-black tracking-tight text-[#4b3b42]">KARIM</h1><p className="mt-2 text-sm font-bold text-[#9a806c]">SMKN 2 Cilaku · online</p></div></div><div className="absolute bottom-7 text-[11px] font-bold text-[#b39a82]">tap into the conversation</div></div></div>;
  }

  return <div className="flex h-screen w-full flex-col overflow-hidden bg-[#f6e7c7] font-sans antialiased text-[#493f43] select-none"><ChatHeader karimArtworkUrl={karimArtworkUrl} isTyping={isTyping} relationship={relationship} currentEvent={currentEvent} audioEnabled={audioEnabled} onToggleAudio={() => setAudioEnabled(!audioEnabled)} onOpenProfile={() => setIsProfileOpen(true)} onOpenRelationship={() => setIsRelationshipOpen(true)} onOpenEvents={() => setIsEventsOpen(true)} onOpenSettings={() => setIsSettingsOpen(true)} /><main className="flex min-h-0 w-full flex-1 flex-col overflow-hidden"><MessageList messages={messages} isTyping={isTyping} karimArtworkUrl={karimArtworkUrl} playerName={playerProfile.name} /><MessageInput onSendMessage={handleSendMessage} disabled={isTyping} onOpenEvents={() => setIsEventsOpen(true)} currentPeriodName={currentEvent.period} /></main><KarimProfileModal isOpen={isProfileOpen} onClose={() => setIsProfileOpen(false)} karimArtworkUrl={karimArtworkUrl} onUpdateArtwork={(url) => setKarimArtworkUrl(isRasterArtwork(url) ? url : undefined)} memories={memories} relationship={relationship} /><RelationshipModal isOpen={isRelationshipOpen} onClose={() => setIsRelationshipOpen(false)} relationship={relationship} /><SchoolEventsModal isOpen={isEventsOpen} onClose={() => setIsEventsOpen(false)} currentEvent={currentEvent} onSelectEvent={handleSelectEvent} /><SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} playerProfile={playerProfile} onUpdateProfile={(p) => setPlayerProfile(p)} audioEnabled={audioEnabled} onToggleAudio={() => setAudioEnabled(!audioEnabled)} autoInitiateEnabled={autoInitiateEnabled} onToggleAutoInitiate={() => setAutoInitiateEnabled(!autoInitiateEnabled)} onResetGame={handleResetGame} /></div>;
}
